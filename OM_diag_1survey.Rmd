...
repfile <- "OM_scenarios/mod_test_11/hydra_sim.rep"
...
est_abundance <- (output$EstNsize) %>% 
     as_tibble() %>%  
     pivot_longer(cols = everything(), names_to = "ilen", names_prefix = "V") %>% 
     mutate(species = rep(hydraDataList$speciesList, each = hydraDataList$Nyrs * nlen * stepperyr),
            year = rep(rep(1:(hydraDataList$Nyrs * stepperyr), each = nlen), length(hydraDataList$speciesList)),
            year = (1 - 1 / stepperyr) + year / stepperyr) %>%
     rename(abundance = value)  # Rename value to abundance for clarity
 
M_abund <- est_M %>%
     left_join(est_abundance, by = c("year", "species", "ilen")) %>%
     rename(M = value)  # Rename columns after join


M_tot <- M_abund %>%
  group_by(year, species) %>%
  summarise(total_M = sum(M * abundance, na.rm = TRUE) / sum(abundance, na.rm = TRUE), .groups = "drop") %>%
...
M_tot %>% 
  ggplot() +
  aes(x = year, y = total_M, col = species) +  # Se usa species para color en vez de bin
  geom_line() +
  facet_wrap(~species, scales = "free") +
  theme_minimal() +
  labs(x = "Year",
       y = "M",
       col = "Species",
       title = "Time series of total natural mortality")
```
