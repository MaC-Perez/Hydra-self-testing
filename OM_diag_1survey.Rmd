...
obs_diet <- hydraDataList$observedSurvDiet

colnames(obs_diet) <- c("survey", "year", "species", "sizebin",
                          "inpN", "wt_prey_1", "wt_prey_2", 
                          "wt_prey_3", "wt_prey_4", "other")

diet_combined <- obs_diet %>%
  group_by(year, species, sizebin) %>%
  mutate(w = inpN) %>%
  summarise(
    InpN = sum(w),
    across(starts_with("wt_prey_"),
           ~ weighted.mean(.x, w, na.rm = TRUE)),
    other = weighted.mean(other, w, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(survey = 1) %>%
